...
/**
 *   "출고조회 - 세종" 에서 호출
 *   : order_no,bar_cd 로, 이미 검수완료되었는지 체크
 *
 */
...
if(empty($orderNo) || empty($barCd)){
    ajaxExitJson("필수 인자가 누락되었습니다.(v) ");
}
...
$data = sql_fetch($sql);
if($data)
    ajaxEchoJson(true,"이미 검증 완료된 도서입니다. ");
else
    ajaxEchoJson(false,"미 검수된 도서입니다. ");
