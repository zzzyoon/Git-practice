...
////bylee 발신 전화번호 . 20200501
$mb_call_mapping_number         = isset($_POST['mb_call_mapping_number'])           ? trim($_POST['mb_call_mapping_number'])         : "";
...
//bylee 발신 전화번호 . 20200501
$mb_call_mapping_number = clean_xss_tags($mb_call_mapping_number );
...
if ($w == '' || $w == 'u') {
    ...
    if($is_school_member){
        if(empty($mb_tel) || empty($mb_2)){
            alert('대표전화번호와 문자 회신번호는 필수 입력 정보입니다. ');
        }
    }
...
if ($w == '') {
    $sql = " insert into {$g5['member_table']}
                set mb_id = '{$mb_id}',
                     ...
                     mb_email = '{$mb_email}',
                     mb_call_mapping_number  = '{$mb_call_mapping_number}',
    ...
    $sql = " update {$g5['member_table']}
                set mb_nick = '{$mb_nick}',
                    ...
                    mb_signature = '{$mb_signature}',
                    mb_2 = '{$mb_2}',
                    mb_upd_id='{$member['mb_id']}',
                    mb_upd_datetime=sysdate(),
                    mb_profile = '{$mb_profile}'
                    {$sql_password}
                    {$sql_nick_date}
                    {$sql_open_date}
    ...
    $gsql = " update  {$g5['member_group_table']} set
              mg_1 = '$mb_tel',
              mg_2 = '$mb_2'
              where mg_no = '{$member['mb_1']}'
      ";
