...
//    exit;
    ajaxExitJson("내용에 올바르지 않은 코드가 다수 포함되어 있습니다. ");
}
...
if (!$is_admin && !$board['bo_use_secret'] && (stripos($_POST['html'], 'secret') !== false || stripos($_POST['secret'], 'secret') !== false || stripos($_POST['mail'], 'secret') !== false)) {
	$msg = '비밀글 미사용 게시판 이므로 비밀글로 등록할 수 없습니다.';
    ajaxExitJson($msg);
}
...
if ($w == '' || $w == 'r') {
    if (isset($_SESSION['ss_datetime'])) {
        if ($_SESSION['ss_datetime'] >= (G5_SERVER_TIME - $config['cf_delay_sec']) && !$is_admin) {
            $msg = ('너무 빠른 시간내에 게시물을 연속해서 올릴 수 없습니다.');
            ajaxExitJson($msg);
        }
    }
...
if ($w == '' || $w == 'r') {
    ...
    if (get_session('ss_bo_table') != $_POST['bo_table'] || get_session('ss_wr_id') != $_POST['wr_id']) {
        ...
        ajaxExitJson("올바른 방법으로 수정해 주십시오.");
    }
    */
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($member['mb_id'] != $group['gr_admin']) // 자신이 관리하는 그룹인가?
            ajaxExitJson('자신이 관리하는 그룹의 게시판이 아니므로 수정할 수 없습니다.');
        else if ($member['mb_level'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            ajaxExitJson('자신의 권한보다 높은 권한의 회원이 작성한 글은 수정할 수 없습니다.');
    } else if ($is_admin == 'board') { // 게시판관리자이면_______
        $mb = get_member($write['mb_id']);
        if ($member['mb_id'] != $board['bo_admin']) // 자신이 관리하는 게시판인가?
            ajaxExitJson('자신이 관리하는 게시판이 아니므로 수정할 수 없습니다.');
        else if ($member['mb_level'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            ajaxExitJson('자신의 권한보다 높은 권한의 회원이 작성한 글은 수정할 수 없습니다.');
    } else if ($member['mb_id']) {
        if ($member['mb_id'] != $write['mb_id'])
            ajaxExitJson('자신의 글이 아니므로 수정할 수 없습니다.');
    } else {
        if ($write['mb_id'])
            ajaxExitJson('로그인 후 수정하세요.');
    }
...
if($w == 'u') {
    ...
    if($file_count && (int)$file['count'] > $board['bo_upload_count']) {
        $msg = ('기존 파일을 삭제하신 후 첨부파일을 ' . number_format($board['bo_upload_count']) . '개 이하로 업로드 해주십시오.');
        ajaxExitJson($msg);
    }
} else {
    if($file_count > $board['bo_upload_count']) {
        $msg = ('첨부파일을 ' . number_format($board['bo_upload_count']) . '개 이하로 업로드 해주십시오.');
        ajaxExitJson($msg);
    }
}
...
if ($file_upload_msg) {
    ...
    ajaxEchoJson(true,$file_upload_msg);
} else {

    // 원코드 > 글쓰기 후 페이지 이동
    //goto_url(G5_HTTP_BBS_URL . '/board.php?bo_table=' . $bo_table . '&amp;wr_id=' . $wr_id . $qstr);

    $msg = "정상적으로 게시글을 등록했습니다. ";
    if($w=="u")
        $msg="정상적으로 게시글이 수정되었습니다. ";


    //ajaxEchoJson(true,$msg);
     $data = sql_fetch("select * from  {$write_table} where wr_id = '{$wr_id}' ");
     if($data) {
         $data['bo_table'] = $bo_table;
         $data['wr_datetime_w'] = wellKnownDatetimeFormat($data['wr_datetime']);

         if($data['wr_1']){
             $data['wr_1_nm'] = get_member_group($data['wr_1'])['mg_name'];
         } else {
             $data['wr_1_nm'] = "";
         }

         if($data['wr_2']){
             $data['wr_2_nm'] = getAgencyName($data['wr_2']);
         } else {
             $data['wr_2_nm'] = "";
         }
         ...
         $data['wr_8_w'] = wellKnownDatetimeFormat($data['wr_8']);
         $data['wr_6_nm'] = "대기";
         if($data['wr_6']=="E")
             $data['wr_6_nm']="처리중";
         else if($data['wr_6']=="F")
    ...
    ajaxEchoJsonObject(true,$msg,$data);
