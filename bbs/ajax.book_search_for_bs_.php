...
if(!empty($goodCd) || !empty($sizeGb) || !empty($locCd)){
    ...
    $sizeGb = filteringEanCode($sizeGb);
...
$sql = "begin CAPOPDA_CAPO1.SP_SRCHPRDJG_CAPO(:p_good_cd,:p_size_gb,:p_loc_cd,:cur_jglst); end; ";
...
while ($row = oci_fetch_array($cursor, OCI_RETURN_LOBS )) {
    ...
    $rowUid = $row['good_cd']."_".$row['size_gb']."_".$row['loc_cd']; //primary key 돌겠다 ㅡㅡ;
