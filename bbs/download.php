...
// ios download bug 때문에 ㅡㅡ;
if (!is_mobile() && !get_session('ss_view_'.$bo_table.'_'.$wr_id))
    alert('잘못된 접근입니다.');
...
} else if (preg_match("/Safari/i", $_SERVER['HTTP_USER_AGENT'])){ //bylee bugfix . ios safari browser 한글이름 깨짐
    header("content-type: file/unknown");
    header("content-length: ".filesize("$filepath"));
    header("content-disposition: attachment; filename=\"".basename($file['bf_source'])."\"");
    header("content-description: php generated data");
