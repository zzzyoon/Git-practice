...
<div id="bo_list">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            ...
            <td class="td_subject">
                ...
                <div class="d-block d-sm-none">
                    ...
                    <span class="badge badge-secondary"><?php echo $list[$i]['num'];?></span>

                    <?php
                    echo $list[$i]['icon_reply'];
                    if ($is_category && $list[$i]['ca_name']) {
                        ?>
                        <a href="<?php echo $list[$i]['ca_name_href'] ?>" class="bo_cate_link_m"><?php echo $list[$i]['ca_name'] ?></a>
                        <br>
                    <?php } ?>
                    <a href="<?php echo $list[$i]['href'] ?>">
                        <?php echo $list[$i]['subject'] ?>
                        <?php if ($list[$i]['comment_cnt']) { ?>
                            <span class="sound_only">댓글</span><span class="cnt_cmt">+<?php echo $list[$i]['comment_cnt']; ?></span><span class="sound_only">개</span>
                        <?php } ?>
            ...
            <td class="td_name sv_use align-top"><?php echo $list[$i]['name'] ?></td>
        ...
        <?php } ?>
        <?php if (count($list) == 0) { echo '<tr><td colspan="'.$colspan.'" class="empty_table text-center" style="height:100px">게시물이 없습니다.</td></tr>'; } ?>
        </tbody>
        </table>
    </div>
...
<?php
if(!is_mobile())
    echo cm_bootstrap_paging($write_pages);
else
    echo "
<div class='d-flex justify-content-center'>
    <div class='more_button btn btn-primary text-center w-75 mt-1 mb-2 ml-3 mr-3' >+ 더보기</div>
</div>
";
?>
</div>
...
<!-- new 페이지 / more버튼 형식 -->
<input type="hidden" id="page_n" value="2">
<input type="hidden" id="cur_page_n" value="<?php echo $page ?>">
<input type="hidden" id="total_page_n" value="<?php echo $total_page ?>">
    ...
    $( document ).ready(function(){
        ...
        $(".more_button").click(function(){
            ...
            $.get( "board.php?bo_table=<?=$bo_table?>&ajax_ck=1&sca=<?php echo urlencode($sca) ?>&sop=and&sst=<?=$_GET['sst']?>&sod=desc&page="+page_n, function( data ) {
                ...
                var append_data = $( data ).find('#ajax_board_list').html();
                var empChk = $( data ).find('.empty_table');
                $(".more_button").html( '+ 더보기' );

                if(empChk.length == 0){
                    $('#ajax_board_list').append(append_data);
                    $('#page_n').val(parseInt(page_n)+1);
                } else {
        ...
        var totalPage = $('#total_page_n').val();
        var curPage = $('#cur_page_n').val();
