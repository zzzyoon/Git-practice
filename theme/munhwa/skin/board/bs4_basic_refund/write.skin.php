...
if(!empty($w) && !$is_admin && $write['ca_name']!='대기'){
    alert(' ['.$write['ca_name'].'] 상태이므로 본문은 수정불가능합니다. \n *댓글을 이용해주세요.');
}
...
if(!$w) {
    ...
    if ($is_school_member) {
        $is_readyonly="readonly";
        $gmember = get_member_group($member['mb_1']);
        $subject = sprintf("%s - 학교회원 환불신청",$member['mb_name']);

    } else if ($is_teacher_member) {
        $is_readyonly="readonly";
        $gmember = get_member_group($member['mb_1']);
        $subject = sprintf("%s / %s - 교직원회원 환불신청",$gmember['mg_name'],$member['mb_name']);

    } else if(!$is_admin) {
        alert("환불 게시판은 작성 권한이 없습니다. ");
    }

}

// timepicker {
add_stylesheet('<link rel="stylesheet" href="'.G5_THEME_JS_URL.'/datepicker.min.css?vs=1">', 1);
add_javascript('<script src="'.G5_THEME_JS_URL.'/datepicker.min.js"></script>', 10);
add_javascript('<script src="'.G5_THEME_JS_URL.'/datepicker.kr.js"></script>', 10);
...
<section id="bo_w">
    ...
    <?php if ($w == "u" && $is_admin && $is_category) { ?>
	<div class="form-group">
	    <label for="ca_name" class="font-weight-bold">분류(처리상태)</label>
	    <select name="ca_name" id="ca_name" class="form-control  bg-primary text-light w-25">
    ...
    <?php if($w == "u" && $is_admin) { ?>
        ...
        <!--
        <div class="form-group label-floating">
            <label class="control-label font-weight-bold" for="wr_1">환불완료 금액 </label>
            <input type="text" class="form-control bg-primary text-light w-25" id="wr_1" name="wr_1" value="<?php if($w=="u"){echo$write['wr_1'];} ?>">
        </div>

        <div class="form-group label-floating">
            <label class="control-label font-weight-bold" for="wr_2">환불완료 일자 </label>
            <input type="text" class="form-control bg-primary text-light w-25" data-time-format='hh:ii:00' id="wr_2" name="wr_2" value="<?php if($w=="u"){echo$write['wr_2'];} ?>">
        </div>
            -->
	...
	<div class="form-group label-floating">
		...
		<input type="text" class="form-control" name="wr_subject" <?php echo $is_readyonly?> id="wr_subject" value="<?php echo $subject ?>">
    ...
    $(function(){
        ...
        <?php if($is_admin && $w=="u") { ?>
        // 환불완료 일자
        $('#wr_2').datepicker({language:'kr',timepicker: true,
            position:'top center',
            minDate: new Date(),
            ...
            todayButton: true,
            navTitles: {
                days: '<h5>환불완료 시간</h5> &nbsp; MM, yyyy'
            }
        });

        // timepicker > 닫기 버튼 역활
        $('span[data-action="today"]').click(function(){
            $('#btn_submit').focus();
        });
