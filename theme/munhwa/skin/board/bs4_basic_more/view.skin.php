...
$extraMemInfo="";
if($is_admin_member || $is_cs1_member || $is_cs2_member) {
    if ($view['mb_id']) {

        $extraMemInfo.="<div class='alert alert-secondary my-2 text-right rounded-pill'>";
        $mb = get_member($view['mb_id']);
        if (!empty($mb['mb_1'])) {
            $gmb = get_member_group($mb['mb_1']);
            $extraMemInfo .= "소속 학교 : <a href='/adm/member_grp_list.php?sfl=mg_no&stx={$gmb['mg_no']}' target='_blank' class='btn btn-sm btn-primary rounded-pill mr-2 px-3'>{$gmb['mg_name']}</a>";
        }


        $extraMemInfo .= " 아이디 : <a href='/adm/member_teacher_form.php?w=u&mb_id={$view['mb_id']}' target='_blank' class='btn btn-sm btn-secondary rounded-pill text-light px-3'>{$view['mb_id']}</a>";
        $extraMemInfo.="</div>";
    }
...
<article id="bo_v" style="width:<?php echo $width; ?>">
    <header>
        <h1 id="bo_v_title">
            <?php
            if ($category_name) echo "<strong>{$view['ca_name']}</strong> | "; // 분류 출력 끝
