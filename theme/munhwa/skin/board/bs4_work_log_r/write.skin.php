...
add_stylesheet('<link rel="stylesheet" href="'.G5_THEME_JS_URL.'/notific.css">', 11);

//moment.js include
add_javascript('<script src="'.G5_JS_URL.'/moment-with-locales.min.js"></script>', 12);


//add_javascript('<script src="'.G5_THEME_JS_URL.'/toastr.js"></script>', 15); // ie11 error
//add_javascript('<script src="'.G5_THEME_JS_URL.'/socket.io.js"></script>', 16);  //3.0 ie11 error
add_javascript('<script src="'.G5_THEME_JS_URL.'/socket.io23.js"></script>', 16); // socketio.js 2.3 > ie11 stable
// 2.3 버젼은 io 서버 접속시, cors 에러가 발생한다.



//bylee . calamansi .js
add_javascript('<script src="'.$board_skin_url.'/calamansi.js"></script>', 22);

add_javascript('<script src="'.G5_THEME_JS_URL.'/notific.js"></script>', 24); // ie11 error
  ...
  -->
        ...
        <div class="row" >
            <div class="col-md-6 form-group label-floating">
                ...
                <?php } ?>
                <?php echo str_replace("height:300px","height:200px",str_replace('class=""', 'class="form-control rounded-1 wr_content bg-gray-0" id="wr_content" rows="6" ', $editor_html)); // 에디터 사용시는 에디터로, 아니면 textarea 로 노출 ?>
            ...
            <div class="col-md-6">
                ...
                <textarea id="wr_10" name="wr_10" class="form-control rounded-1 bg-gray-0"  rows="6" style="width:100%;height:200px"><?php echo $write['wr_10']?></textarea>
            </div>
...
<!---->
        ...
        <div class="text-center mb-4 mt-4">
            ...
            <button type="submit" id="btn_submit" accesskey="s" class="btn btn-sm btn-primary "><i class="fas fa-check"></i>
                <?php if($w=="u"){ ?>
                    수정완료
                <?php } else { ?>
                    작성완료
...
<script>
    ...
    // 좌측 새글 알림창 관리(toast box)
    var alertQueue = new Queue();
    var alertBoxLimit=10;

    var socket = io.connect('<?php echo G5_NODE_SERVER_URL?>/article',{secure:true}); //article > namespace 접근
    socket.on('connect', function() {
       // console.log('Node Server connected ____');
    });
    ...
    socket.on('post',function(data){
        ...
        if(data.bo_table == "cs_bbs") {
            // show ToastBox ____________________________________
            var msg = data.wr_datetime + "<br>" + data.wr_3 + " | " + phone_format(data.wr_4);
            if (data.wr_1.length > 0)
                msg += "<br><span class='bg-light rounded-pill px-2 text-secondary my-2 text-size-4' >" + data.wr_1_nm + "</span>";

            if (data.wr_2.length > 0)
                msg += "<br><span class='bg-light rounded-pill px-2 text-secondary my-2 text-size-4' >" + data.wr_2_nm + "</span>";

            msg += "<br><span class='text-danger'>" + data.wr_6_nm + "</span> . " + data.ca_name;
            msg += ' <a class="text-light" href="/bbs/board.php?bo_table=' + data.bo_table + '&wr_id=' + data.wr_id + '" target="_blank" ><i class="fas fa-external-link-square-alt"></i></a>';

            toastBox = Message.add(msg,
                {
                    type: 'info',
                    sticky: true,
                    selector: '.container',
                    vertical: 'top',
                    horizontal: 'left',
                    skin: 'large ico-medium',
                    insert: 'prepend'
                }  //vertical:'top',horizontal:'left',
            );

        } else {  // qa 게시판, certi 게시판  //////////////////////////////////////////////////////////////////
            var msg = "";
            if(data.bo_table == "qa"){
                msg+="#문의하기 게시판";
            } else if(data.bo_table == "certi") {
                msg+="#통신이용 증명원 게시판";
            } else {
                msg+="#기타 게시판";
            }
            ...
            msg += "<br>"+data.wr_datetime;


            msg+="<br><span class='bg-light rounded-pill px-2 text-secondary my-2 text-size-4' >"+makeShortText(data.wr_subject,20)+"</span>";
            msg += "<br><span class='text-primary'>" + data.mg_name + " | "+data.mb_name+" 등록 </span>";
            msg += ' <a class="text-primary" href="/bbs/board.php?bo_table=' + data.bo_table + '&wr_id=' + data.wr_id + '" target="_blank" ><i class="fas fa-external-link-square-alt"></i></a>';
        ...
        alertQueue.enqueue(toastBox);
        if(alertQueue.getSize() > alertBoxLimit){
            var oldBox = alertQueue.dequeue();
            Message.close(oldBox,1000);
        }
                ...
                success: function(data) {
                   ...
                   //console.log(data);
                    ...
                    if(json.result_stat) {
                        ...
                        //alert > toastbox 대체
                        showAlertToast(json.result_msg);

                        if(writeMode=="u"){
                            socket.emit('update', json.result_obj);
                            location.href='/bbs/board.php?bo_table=<?=$bo_table?>';
                        } else {

                            socket.emit('post', json.result_obj);

                            $('#fwrite')[0].reset(); //hidden field를 clear 하지 않는다.
                            $('#wr_1').val(''); //학교 no
                            $('#wr_2').val(''); //대행사 no
                            selectButtonGroup('wr_6', 'R'); //처리상태
                            selectButtonGroup('ca_name', '기타'); //분류
                        ...
                        //alert > toastbox 대체
                        showErrorToast(json.result_msg);
                    }
...
<script  language="javascript"  >
    ...
    function showCallToast(boTable,phoneNumber){
        ...
        $.getJSON(actionUrl,{'bo_table':boTable,'phone_number':phoneNumber},function(json){
            ...
            toastMsg="<span class='bg-light text-dark px-2 py-1 rounded-pill text-size-4'>"+toastTitle+"</span><br>"+toastMsg;

            Message.add(toastMsg,
                {type:'warning',vertical:'bottom',horizontal:'right',skin:'large',insert:'prepend',life:5000}
            );
    ...
    function showAlertToast(msg){
        ...
        var toastMsg = '<div class="text-left py-1"><strong class="bg-light text-dark px-2 rounded-pill">알림</strong></div>'+msg;
        Message.add(toastMsg,
            {type:'success',vertical:'center',horizontal:'center',skin:'large',life:3000,close:false}
        );
    ...
    function showErrorToast(msg){
        ...
        var toastMsg = '<div class="text-left py-1"><strong class="bg-light text-dark px-2 rounded-pill">확인</strong></div>'+msg;

        Message.add(toastMsg,
            {type:'error',vertical:'center',horizontal:'center',skin:'large',life:4000,close:false}
        );
