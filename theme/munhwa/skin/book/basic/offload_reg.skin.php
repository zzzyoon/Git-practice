...
<!--            </div>-->
            ...
            <!-- input group area/ subl_no 찾기 아이콘 붙임   -->
            <div class="input-group col-9">
                <input type="text" name="subl_no" value="" id="subl_no" class="form-control" inputmode="search" placeholder="스캔">
        ...
        <div class="form-group row  no-gutters my-1">
            <label for="acceptor_name" class="col-3 col-form-label">인수자</label>
            <div class="col-3">
                <input type="text" class="form-control required" name="acceptor_name" id="acceptor_name"  placeholder="">
            </div>
        </div>
...
<script>
   ...
   // var SCAN_CNT = 0;
    ...
    $(function() {
        ...
        $('#acceptor_name').keyup(function(e){
            if (e.keyCode != 13) {
                return true;
            }
    ...
    function transferConfirm(){
        ...
        if(taxNo.length == 0 || totCnt.length == 0){
            playBeepSound();
            MsgBox.alert("하차등록 할 자료가 존재하지 않습니다.");
            return false;
        }

        MsgConfirmBox.show("하차등록 자료를 전송하시겠습니까?",function(res){
            if(res){
                transferData();
            }
        });
    ...
    function transferData(){
        ...
        var actionUrl = "<?php echo G5_BBS_URL?>/ajax.offload_reg_update.php";
...
//        return true; //for debugging
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                playBeepSound();
                alert("하차등록 자료 전송시 오류가 발생했습니다. ("+error+")");
            }
        }); //end ajax func ====
