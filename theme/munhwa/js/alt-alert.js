...
$(function(){ //bylee
$('body').append(`
<div class="modal bs-example-modal-sm" id="alert" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog mt-n4 modal-dialog-centered modal-sm">
        <div class="modal-content" style="border-radius: 10px">
            <div class="modal-body text-center pl-3 pr-3">
                <div class="text-center d-flex justify-content-center">
                    <div style="width: 25vh" class="logo-alert p-0 m-0"></div>
                </div>
                <div id="alert-message" class="mt-2"></div>
                <div class="pb-1">
                    <button class="btn d-none btn-primary px-4 font-weight-bold" autofocus id="confirmAlert" type="button">Got it</button>
                    <button class="btn d-none btn-secondary" id="cancelAlert" type="button">Cancel</button>
                </div>
            </div>
        </div>
    </div>
...
class AltAlert {
        ...
        return new Promise((resolve,reject) => {
            ...
            $('#confirmAlert').click(function(e){ // 확인 버튼
                ...
                if (data.stop) {
                    ...
                    //$('#alert').addClass('fade').modal('show');
                    $('#alert').modal('hide') //bylee bugfix
                }
                else {
                    ...
                    setTimeout(() => {
                        ...
                        console.log("alet close __________");
                ...
                */
                ...
                //$('#alert').modal('hide') //bylee bugfix
                resolve(true) //promise return
            })
            $('#cancelAlert').click(function(e){ // 취소 버튼
                lottie.setDirection(-1)
                lottie.setSpeed(2.0)
                lottie.play()
    ...
    alertHandler(data) {
        ...
        //    data.showConfirmButton = true  //bylee bugfix 20210201
        //}



        let confirmButton = $('#confirmAlert')
        let cancelButton = $('#cancelAlert')
        if (data.status == 'loading') {
            ...
            $('#alert').addClass('fade').modal(outsideClick)

            lottie.destroy()
            this.alert('loading','loading')
        }
        else {
            this.alert('loading',0)
            this.alert(data.status,1)
        }

        if (data.status != 'loading') {
            if (data.showConfirmButton == true) {
                confirmButton.removeClass('d-none')
                confirmButton.attr('focus')
            }
            else {
                confirmButton.addClass('d-none')
            }
        ...
        confirmButton.text(data.confirmButtonText ? data.confirmButtonText : 'Okay')
        cancelButton.text(data.cancelButtonText ? data.cancelButtonText : 'Cancel')
        confirmButton.css('background-color',data.confirmButtonColor).css('border-color',data.confirmButtonColor)
        cancelButton.css('background-color',data.cancelButtonColor).css('border-color',data.cancelButtonColor)
        document.getElementById('alert-message').innerHTML =  `
            <h3 class="font-weight-bold mt-n2 text-capitalize text-center" id="alert-title">${data.title == undefined ? '' : data.title}</h3>
            <p class="text-center font-weight-bold" id="alert-text">${data.text == undefined ? '' : data.text}</p>
        `
    }
    ...
    alert(icons,status) {
        ...
        lottie.setSpeed(2.5)
        lottie.play()
        let bm = document.getElementsByClassName('logo-alert')
        Array.prototype.forEach.call(bm,icon => {

            var anim = lottie.loadAnimation({
                container: icon,
                animationData: icons == 'success' ? this.success : icons == 'error' ? this.error : icons == 'warning' ? this.warning : icons == 'loading' ? this.loading : icons == 'question' ? this.question : '',
                renderer: 'svg',
                loop: status == 'loading' ? true : false,
                autoplay: true,
            })
...
// instance alert class
const Alt = new AltAlert
...
const MsgBox = {
    ...
    warningCallback:function(msg,callback){
       return Alt.alternative({
            status:'warning',
            title:'경고',
            text:msg,
    ...
    errorCallback:function(msg,callback){
        return  Alt.alternative({
            status:'error',
            title:'오류',
            text:msg,
    ...
    failCallback:function(msg,callback){
        return  Alt.alternative({
            status:'error',
            title:'실패',
            text:msg,
    ...
    succCallback:function(msg,callback){
      return  Alt.alternative({
            status:'success',
            title:'알림',
            text:msg,
    ...
    confirm:function(msg,callback){
       ...
       return Alt.alternative({
            ...
            confirmButtonText:'확인',
            cancelButtonText:'취소',
            stop:false,  // true > confirm >ok 버튼을 눌러도 창이 닫아지지 않는다. > 상차등록 전송시, false 로 하니, 처리 결과후애 succCallback 호출했음에도 자동닫혀버린다.
            title:'확인',
            text: msg
        }).then(callback);
