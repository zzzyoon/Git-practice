...
function remove_cookie($cookie_name){
    setcookie(md5($cookie_name), "", G5_SERVER_TIME - 3600, '/', G5_COOKIE_DOMAIN);
}
...
//pure 쿠키변수 생성
/*
function set_pure_cookie($cookie_name, $value, $expire)
{
    global $g5;
    setcookie($cookie_name, base64_encode($value), G5_SERVER_TIME + $expire, '/', G5_COOKIE_DOMAIN);
...
function set_pure_cookie($cookie_name, $value, $expire)
{
    ...
    $encVal = base64_encode($value);

    if(substr($encVal,-1) == "=") {
      //  $encVal = substr($encVal, 0, strlen($encVal) - 1);
        $encVal = str_replace("=","",$encVal); // 끝자리 == 두개 오는경오 발생 (예 > dGNhbGxfMjIxNDY2NTAxMw== )
    }

    setcookie($cookie_name, $encVal, G5_SERVER_TIME + $expire, '/', G5_COOKIE_DOMAIN);

}

//bylee
function remove_pure_cookie($cookie_name){
    setcookie($cookie_name, "", G5_SERVER_TIME - 3600, '/', G5_COOKIE_DOMAIN);
}



//pure 쿠키변수 생성
function get_pure_cookie($cookie_name)
{
    global $g5;
...
{
    global $g5, $qstr,$client_type; //bylee $client_type
    ...
    while ($row = sql_fetch_array($result))
    {
        ...
        if($client_type == "ios")
            $file[$no]['href'] = "js2ios://Down?param1=".G5_BBS_URL."/download.php?bo_table=$bo_table&amp;wr_id=$wr_id&amp;no=$no" . $qstr;
        else
...
function get_member($mb_id, $fields='*')
{
    ...
    //bylee
    $member = sql_fetch(" select $fields from {$g5['member_table']} where emp_no = TRIM('$mb_id') ");
    if($member) {
        $member['mb_id'] = $member['emp_no'];
        $member['mb_name'] = $member['name'];
        $member['mb_level'] = 3;
        $member['mb_password'] = $member['passwd'];
        $member['mb_datetime'] = $member['upd_date'];
    }
...
// BYLEE//////////////////////////////////////////////////////////////////////////////////////////////////
// 회원 그룹(학교) 정보 추출
function get_member_group($mg_no, $fields='*')
{

    global $g5;
    $data = sql_fetch(" select $fields from {$g5['member_group_table']} where mg_no = TRIM('$mg_no') ");

    if(!empty($data['mg_3']))
        $data['mg_agency_code'] = $data['mg_3'];
...
//bylee
// 050 안심번호 > 메타 정보(master table)
function get_safety_number($grp_no, $fields='*')
{

    global $g5;
    return sql_fetch(" select $fields   from {$g5['safety_meta_table']} where sm_grp_no = TRIM('$grp_no') limit 1 ");
...
// bylee
// 회원 그룹을 SELECT 형식으로 얻음
function get_member_group_select($name, $selected='', $event='')
{
    global $g5, $is_admin, $member;

    $sql = " select  *  from {$g5['member_group_table']} a ";

    $sql .= " order by mg_name asc ";

    $result = sql_query($sql);
    $str = "<select id=\"$name\" name=\"$name\" $event>\n";
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        if ($i == 0) $str .= "<option value=\"\">선택</option>";
        $str .= option_selected($row['mg_no'], $selected, $row['mg_name']." / ".$row['mg_cmt']." / ".date("Y-m-d", strtotime($row['mg_datetime'])));
    }
    $str .= "</select>";
    return $str;
}
...
// DB 연결
function sql_connect($host,  $user, $pass, $port=1521, $sid="orcl",$charset = G5_ORACLE_CHARSET)
{
    global $g5;

    $connStr = sprintf("(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=%s) (PORT=%d)))(CONNECT_DATA=(SERVER=DEDICATED)(SID=%s)))",
        $host,$port,$sid);


    $connect_db = oci_connect($user,$pass,$connStr,$charset);
    oci_set_call_timeout($connect_db, 3000);

    if (!$connect_db) {
        $errs = oci_error();
        die('DB Connect Error: '.$errs['code']." > ".$errs['message']);
        return false;
    }
...
function sql_query($sql, $error=G5_DISPLAY_SQL_ERROR, $link=null,$isTransaction=false)
{
    global $g5;
    ...
    if($isTransaction)
        $transMode=OCI_DEFAULT;

    $stmt = oci_parse($link,$sql);

        $res = "";
        if ($error) {
            $res = oci_execute($stmt,$transMode) or die("<p>$sql</p>" .  oci_error($link) . "<p>error file : {$_SERVER['SCRIPT_NAME']}</p>");
        } else {
            $res = oci_execute($stmt,$transMode);
        }


    return (!$res)?false:$stmt;

}

function sql_query_parse($sql, $error=G5_DISPLAY_SQL_ERROR, $link=null)
{
    global $g5;
...
function sql_query_execute($stmt, $error=G5_DISPLAY_SQL_ERROR,$isTransaction=false)
{
    global $g5;
;
    // default -> auto commit
    ...
    $res = "";
    if ($error) {
        $res = oci_execute($stmt,$transMode) or die(oci_error() . "<p>error file : {$_SERVER['SCRIPT_NAME']}</p>");
    } else {
        $res = oci_execute($stmt,$transMode);
    }
...
function sql_fetch($sql, $error=G5_DISPLAY_SQL_ERROR, $link=null)
{
    ...
    $stmt = sql_query($sql, $error, $link);
    if(!$stmt)
        return false;


    if($error){
        $row = sql_fetch_array($stmt); //null return  > or > die
        if(oci_error())
            die("<p>$sql<p> error msg : "  .  oci_error() . "<p>error file : {$_SERVER['SCRIPT_NAME']}");
    } else {
...
function sql_result($sql,$link=null){
...
function sql_fetch_array($stmt)
{
    ...
    if($row) {
        $row = array_map_deep('iconv_utf8', $row);  // munhwa db is euckr
        $row = array_change_key_case($row,CASE_LOWER); // to lower case
    }
...
// DML query로 영향미친 rows count 리턴
function sql_num_rows($stmt)
{
        return oci_num_rows($stmt); //parse result 를 parameter 로 넘겨야   = mysql_affected_rows 비슷
}
...
// *참고 : bylee
// 그누보드 기본함수이나, 오류가 발생하지 않아도  " 0: "값을 리턴해서 불편
// 결국 아래의 sql_errorno 함수 생성
/*
...
function sql_errorno($link=null)
{
    ...
    if(function_exists('mysqli_error') && G5_MYSQLI_USE) {
        ...
        if(!empty($err_msg)){
            return mysqli_errno($link) . ' : ' . $err_msg;
        }
    } else {
        $err_msg = mysql_error($link);
        if(!empty($err_msg)) {
            return mysql_errno($link) . ' : ' . $err_msg;
        }
    }
...
//bylee
// 그룹 회원 관리  (테이블에서 추출x, 변수필수 변수만 설정)
function get_gmember_config($fld='*')
{
    global $g5;
    ...
    //$row = sql_fetch($sql);
    $row=array();
    $row['gm_title']="교원 회원관리";//"그룹 회원관리";
    $row['gm_mobile_page_rows']=10;
    $row['gm_page_rows']=10;
    $row['gm_mobile_subject_len'] = 30;
    $row['gm_subject_len']= 60;
    $row['gm_mobile_skin']="theme/basic";
    $row['gm_skin']="theme/basic";
    $row['gm_req_hp']=true;
    $row['gm_req_tel']=true;
    $row['gm_use_email_certify']=false;
    $row['gm_use_addr']=true;
    $row['gm_register_point']=100; // 가입축하금
    $row['gm_email_mb_member']=true;// 가입 환영 메일
...
// 001.  입고 & 적치  메뉴 ///////////////////////////////////////////
function get_book_config($fld='*')
{
    global $g5;
    $row=array();
    $row['bf_title']="";
    $row['bf_mobile_page_rows']=10;
    $row['bf_page_rows']=10;
    $row['bf_mobile_subject_len'] = 30;
    $row['bf_subject_len']= 60;
    $row['bf_mobile_skin']="theme/basic";
    $row['bf_skin']="theme/basic";
...
// 002.  도서 재고 이동  ///////////////////////////////////////////
function get_book_move_config($fld='*')
{
    global $g5;
    $row=array();
    $row['bf_title']="";
...
//bylee
// 문자 관리  (테이블에서 추출x, 변수필수 변수만 설정)
function get_sms_config($fld='*')
{
    global $g5;
    ...
    $row=array();
    $row['sms_title']="문자 관리";
    $row['sms_mobile_page_rows']=15; // 모바일 페이지당 라인수
    $row['sms_page_rows']=15; // 페이지당 라인수

    $row['sms_write_pages'] = 10; //페이지 표시수
    $row['sms_mobile_pages'] = 5; //모바일 페이지 표시수

    $row['sms_mobile_subject_len'] = 30;
    $row['sms_subject_len']= 60;
    $row['sms_mobile_skin']="theme/basic";
    $row['sms_skin']="theme/basic";
    $row['sms_req_hp']=true;
    $row['sms_req_tel']=true;



    //mms 첨부 이미지 사이즈__________
    $row['sms_image_max']= 800;
    $row['sms_image_opt']= 800;
    $row['sms_image_min']= 300;
    $row['sms_max_file_size']= 1048576; //bytes - 1mb = 1048576 bytes
    return $row;
...
function get_call_config($fld='*')
{
    ...
    //$row = sql_fetch($sql);
    $row=array();
    $row['call_title']="안심번호 관리";
    $row['call_mobile_page_rows']=15; // 모바일 페이지당 라인수
    $row['call_page_rows']=15; // 페이지당 라인수

    $row['call_write_pages'] = 10; //페이지 표시수
    $row['call_mobile_pages'] = 5; //모바일 페이지 표시수

    $row['call_mobile_subject_len'] = 30;
    $row['call_subject_len']= 60;
    $row['call_mobile_skin']="theme/basic";
    $row['call_skin']="theme/basic";
...
// bylee . 20200423
// 발신번호 변경 서비스 //////////////////////////////////////
function get_callmapping_config($fld='*')
{
    global $g5;


    $row=array();
    $row['cm_title']="안심발신 서비스";

    $row['cm_mobile_page_rows']=15; // 모바일 페이지당 라인수
    $row['cm_page_rows']=15; // 페이지당 라인수

    $row['cm_write_pages'] = 10; //페이지 표시수
    $row['cm_mobile_pages'] = 5; //모바일 페이지 표시수

    $row['cm_mobile_subject_len'] = 30;
    $row['cm_subject_len']= 60;
    $row['cm_mobile_skin']="theme/basic";
    $row['cm_skin']="theme/basic";
...
//bylee
// 알림 설정
function get_settings_config($fld='*')
{
    global $g5;
    ...
    $row=array();
    $row['settings_title']="알림 설정";
    $row['settings_mobile_page_rows']=15; // 모바일 페이지당 라인수
    $row['settings_page_rows']=15; // 페이지당 라인수

    $row['settings_write_pages'] = 10; //페이지 표시수
    $row['settings_mobile_pages'] = 5; //모바일 페이지 표시수

    $row['settings_mobile_subject_len'] = 30;
    $row['settings_subject_len']= 60;
    $row['settings_mobile_skin']="theme/basic";
    $row['settings_skin']="theme/basic";
...
function member_delete($mb_id)
{
    ...
    /*
    //bylee . 티처콜 > 교직원회원 > 전화번호부  기본그룹 삭제
    sql_query("delete from  {$g5['sms5_book_group_table']}  where mb_id = '$mb_id' ");

    // 전화번호부
    sql_query("delete from {$g5['sms5_book_table']}  where mb_id = '$mb_id' ");



    //bylee . 티처콜 > 학교회원 > 업무시간 정보 삭제
...
//bylee /////////////////////////////
function ajaxEchoJson($stat,$msg){
    $result = array();
    $result['result_stat']=$stat;
    $result['result_msg']=$msg;
    echo json_encode($result);
}

function ajaxExitJson($errMsg){
    ajaxEchoJson(false,$errMsg);
    exit;
}

function convertPlainParams($array){
...
function exitVarDump($params)
{
    echoVarDump($params);
    exit;
}

function echoVarDump($params){
    echo "<pre>";
    var_dump($params);
    echo "</pre>";
}

function echoBox($msg,$isExit=false,$header="*for Debugging"){

    echo'
<div class="card m-4">
  <div class="card-header">'.$header.'</div>
  <div class="card-body bg-danger text-light">'.$msg.'</div>
  <div class="card-footer"> *Time : '.G5_TIME_YMDHIS.'</div>
</div>';
...
///
/// "관리자>관리권한설정"에서 권한 추가된 회원목록(* auth 테이블에 회원정보가 저장되어져 있으나, DB IO 줄이기위해 상수 설정.활용)
/// 비밀글, 비밀댓글 접근에서 활용
function isMgrMember($mb_lv){
    /*
    if (!defined('G5_MGR_MEMBERS') || !$mb_id)
        return false;

    $mems = explode(",",G5_MGR_MEMBERS);
...
//android push notification message
function requestPushNotificationMessage($token, $msgTitle,$msgBody) {

    //fcm > Notification Message Type > 메세지 전송요청

    $url = 'https://fcm.googleapis.com/fcm/send';
    $apiKey = FCM_SERVER_KEY;

    //set message __________
    $msg = array('body' => $msgBody, 'title'  => $msgTitle);

    $payload = array(
        'to'=>$token,
        'priority'=>'high',
        "notification"=>$msg,
        'time_to_live'=>21600000
    );
...
function requestPushNotificationMessageForIOS($token,$msgTitle,$msgBody) {
    //fcm > Notification Message Type > 메세지 전송요청
    ...
    //set message __________
    //$msg = array('title' =>$msgTitle , 'text' => $msgBody, 'sound' => 'default', 'badge' => '1');

    $msg = array('title' =>$msgTitle , 'body' => $msgBody, 'sound' => 'default'); // noti data
    $payload = array('to' => $token,
        'notification' => $msg,
        'data'=>array('msg_type'=>'notification','msg_link'=>""),  // data 하단에 parameters 를 넣어야, ios > userinfo dictionary 데이터로 전달된다.
        'priority'=>'high',
        'time_to_live'=>21600000);
...
///////////////////////////////////////////////////////////////////////////////////
function requestPushMessageGroup($tokens, $msgType,$msgTitle,$msgContent,$msgLink,$msgExtra) {
    ...
    //set message __________
    $msg = array('msg_type' => $msgType, 'msg_title'  => $msgTitle,'msg_content'=>$msgContent, 'msg_link'=>$msgLink, 'msg_extra'=>$msgExtra);
    ...
    //for NotificationMessageType ////////////////////////////////////////////////////
    /*
    $registrationIds = array();
    array_push($registrationIds, $token);
    $fields = array('registration_ids' => $registrationIds,'data' => $payload);
    */
...
// android push message
///////////////////////////////////////////////////////////////////////////////////
function requestPushMessage($token, $msgType,$msgTitle,$msgContent,$msgLink,$msgExtra) {
...
// fcm msgType is Alert(new Window)
// for ios push message
function requestPushMessageForIOS($token,$msgType,$msgTitle,$msgContent,$msgLink,$msgExtra) {
    //fcm > Notification Message Type > 메세지 전송요청
    ...
    $msg = array('title' =>$msgTitle , 'body' => $msgContent, 'sound' => 'default'); // noti data
    $payload = array('to' => $token,
        'notification' => $msg, // userInfo > aps 객체 > alert.title, alert,body 존재
        'data'=>array('msg_type'=>'alert','msg_link'=>"$msgLink",'msg_extra'=>"$msgExtra"),  // data 하단에 parameters 를 넣어야, ios > userinfo dictionary 데이터로 전달된다.
        'priority'=>'high',
        'time_to_live'=>21600000);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
/// FCM > TOPIC 주제를 구독하는 전체 단말기에 메세지 전송요청
///    *참고 : 리턴 되는 형식이 다르게 성공,실패 구분자도 없고 심플하다.  예제 : {"message_id":62985646546544}
//////////////////////////////////////////////////////////////////////////////////////////////
function requestPushMessageByTopic($topic, $msgType,$msgTitle,$msgContent,$msgLink,$msgExtra) {
...
///
/// param : "Y-m-d H:i:s" datetime string
///
function wellKnownDatetimeFormat($datetime,$format="y-m-d H:i"){

    if(!trim($datetime) || $datetime == "0000-00-00 00:00:00")
        return "";

    $retDatetime = date($format,strtotime($datetime));
    $inpTime = strtotime($datetime);

    if($inpTime > time()) { //  예약시간처리 (미래 시간처리)


        if (date("Y-m-d") == date("Y-m-d", $inpTime)) { //동일일자
            $retDatetime = date("H:i분", $inpTime);
        } else if (date("Y-m-d", strtotime("+1 days")) == date("Y-m-d", $inpTime)) { //익일
            $retDatetime = date("내일 H:i분", $inpTime);
        } else if (date("Y") == date("Y", $inpTime)) { //동일년도시
            $retDatetime = date("m-d H:i", $inpTime);
        }

    } else { // 과거 시간처리

        if ($inpTime >= strtotime("-1 minutes")) {
            $retDatetime = "현재";
        } else if ($inpTime >= strtotime("-59 minutes")) {
            $retDatetime = ceil((time() - $inpTime) / 60) . "분 전";
        } else if (date("Y-m-d") == date("Y-m-d", $inpTime)) {
            $retDatetime = date("H:i분", $inpTime);
        } else if (date("Y-m-d", strtotime("-1 days")) == date("Y-m-d", $inpTime)) {
            $retDatetime = date("어제 H:i분", $inpTime);
        } else if (date("Y") == date("Y", $inpTime)) { //current year
            $retDatetime = date("m-d H:i", $inpTime);
        }
    }
...
function makeRequestParams($expParams=""){

    if(!empty($expParams) && !is_array($expParams))
        $expParams=array("$expParams");

    $params = "";
    foreach($_REQUEST as $key => $val){
        $isBreak = false;

        if(is_array($expParams)) {
            foreach ($expParams as $eval) {
                if ($eval == $key) {
                    $isBreak = true;
                    break;
                }
        ...
        $params.=(empty($params))?sprintf("%s=%s",$key,$val):sprintf("&amp;%s=%s",$key,$val);
    }
...
function isAccessHistory(){
    $ref = $_SERVER['HTTP_REFERER'];

    if(empty($ref))
        return false;

    $path      = parse_url($ref, PHP_URL_PATH);       // get path from url
    $extension = pathinfo($path, PATHINFO_EXTENSION); // get ext from path
    $filename  = pathinfo($path, PATHINFO_FILENAME);
    $basename =  pathinfo($path, PATHINFO_BASENAME); // filename.ext


    $dePattern = "/.+_(update|send|check|delete)\\.php$/i";
    if(preg_match($dePattern,$basename)){
        return false;
    } else {
        return true;
    }
...
function ajaxEchoJsonObject($stat,$msg,$obj){
    $result = array();
    $result['result_stat']=$stat;
    $result['result_msg']=$msg;
...
function ajaxExitJsonObject($stat,$msg,$obj){
    ajaxEchoJsonObject($stat,$msg,$obj);
    exit;
}
